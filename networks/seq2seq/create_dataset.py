...
# do not use May13
data_to = pd.read_parquet('/home/oem/Projects/NetDeviceAbnormalDetection/data/Europe_Network_data.parquet')
...
listB = data_to.columns.tolist()
diff = list(set(listB).difference(set(listA)))
diff_columns = pd.DataFrame(columns=diff)
data = pd.concat([data, diff_columns], axis=1)
data = data[data_to.columns]
...
#
# save file
np.save('data/data_newest/m%s_features.npy'%m, inputs)
np.save('data/data_newest/n%s_targets.npy'%n, targets)
np.save('data/data_newest/n%s_labels.npy'%n, labels)
